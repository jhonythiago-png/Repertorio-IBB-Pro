    ...
    for (const strategy of rotation) {
        try {
            ...
            if (cifraElement) {
                ...
                // ── Injetar Tom e Capotraste se não estiverem no <pre> ──────
                // O Cifra Club às vezes coloca essas infos fora do bloco <pre>
                if (!/^\s*Tom\s*:/im.test(cleanedText)) {
                    const tomEl = doc.querySelector('.tom, .cifra-tom, #tom, .js-tom, [class*="tom"]');
                    const tomFull = tomEl ? tomEl.textContent.replace(/^Tom[\s:]*/i,'').trim() : null;
                    if (tomFull) cleanedText = `Tom: ${tomFull}\n` + cleanedText;
                }
                if (!/^\s*Capotraste/im.test(cleanedText)) {
                    const bodyText = doc.body ? (doc.body.innerText || doc.body.textContent) : '';
                    const capoRx = bodyText.match(/Capotraste\s+na\s+(\d+)[aª°]?\s*casa/i);
                    if (capoRx) cleanedText = `Capotraste na ${capoRx[1]}ª casa\n` + cleanedText;
                }
                // ────────────────────────────────────────────────────────────
    ...
    if (capoDisplay && capoGrid) {
        ...
        capoGrid.querySelectorAll('.capo-btn-item').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                const capoText = document.getElementById('perfCapoText');
                if (capoText) capoText.textContent = casa === 0 ? 'Sem capo' : `${casa}ª casa`;
