...
function openPerformance(song, mode) {
    ...
    if (tomLine) {
        ...
        if (formaMatch) {
            ...
            // Se não diz a forma explicitamente, tenta detectar pelos acordes.
            // Essencial quando tem CAPO mas a linha de Tom não especifica a forma.
    ...
    if (song.url || song.vid_id) {
        let finalPerfUrl = song.url || `https://www.youtube.com/embed/${song.vid_id}`;
        if (_playerIframe) _playerIframe.src = finalPerfUrl;
...
async function fetchData() {
    ...
    try {
        ...
        // Filtrar excluídas no JS para evitar problemas com NULL no Supabase
        const activeSongs = data.filter(s => s.status !== 'DELETED');
        
        ALL_CATEGORIES = [...FIXED_CATEGORIES];
        const grouped = {};

        activeSongs.forEach(song => {
            let cats = Array.isArray(song.categories) ? song.categories : (typeof song.categories === "string" ? [song.categories] : ["Geral"]);
            if (!cats || cats.length === 0) cats = ["Geral"];

            // Normalização para evitar duplicidade por erro de digitação/encoding
            const normalizedCats = cats.map(c => {
                let name = String(c).trim();
                if (name.includes('Adorao') || name.includes('AdoraÃ§Ã£o')) name = name.replace(/.*Adora.*/i, "Adoração");
                if (name.includes('Celebrao')) name = "Celebração/Adoração/Louvor";
                if (name.includes('Clssicas')) name = "Clássicas";
                return name;
            });
            ...
            if (song.status === 'NOVA' && !normalizedCats.includes('Novos')) {
                if (!grouped['Novos']) grouped['Novos'] = [];
                if (!grouped['Novos'].find(s => s.id === song.id)) {
        ...
        // Adicionar categoria "Todos os Louvores" ao final para facilitar conferência total
        const allUnique = [];
        ...
        if (allUnique.length > 0) {
            repertoireData.push({ category: "Todos os Louvores", items: allUnique });
        }
...
function openVideo(song) {
    ...
    // Log para diagnóstico (Aperte F12 no navegador para ver)
    console.log("Tentando abrir vídeo:", song.title);
    console.log("URL do banco:", song.url);
    console.log("ID do vídeo:", song.vid_id);

    // Tentar usar a URL do banco diretamente, ou montar se tiver apenas o ID
    let finalUrl = song.url || "";
    if (!finalUrl && song.vid_id) {
        finalUrl = `https://www.youtube.com/embed/${song.vid_id}`;
    }
...
function updateFooter() {
    ...
    if (footer) {
        // Contagem única de músicas
        const allUniqueTitles = new Set();
        repertoireData.forEach(cat => {
            cat.items.forEach(song => allUniqueTitles.add(song.title));
        });
        const total = allUniqueTitles.size;
        
        footer.innerHTML = `
            <p class="footer-dev">Repertório IBB • 2024 • <strong>${total} Louvores</strong></p>
            <p class="footer-dev" style="font-size: 10px; opacity: 0.6; margin-top: 5px;">Desenvolvido por Jhony Beraldo</p>
        `;
    }
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    let _setlistManualSemitones = 0;

    function _updateSetlistKeyDisplay() {
        const display = document.getElementById('setlistTomDisplay');
        const formaLabel = document.getElementById('setlistFormaLabel');
        const capoDisplay = document.getElementById('setlistCapoDisplay');
        if (!display || !formaLabel) return;

        const initialKey = display.dataset.initialKey || 'C';
        const quality = display.dataset.initialQuality || '';
        const capo = parseInt(capoDisplay.dataset.capo || 0);

        const fIdx = SCALE.indexOf(normalizeKey(initialKey));
        if (fIdx === -1) return;

        // Tom Forma = Tom Inicial + Manual Semitones
        const formaIdx = (fIdx + _setlistManualSemitones + 24) % 12;
        const formaKey = SCALE[formaIdx];

        // Tom Real = Tom Forma + Capo
        const realIdx = (formaIdx + capo + 24) % 12;
        const realKey = SCALE[realIdx];

        display.textContent = realKey + quality;
        display.dataset.key = realKey; // Guarda o tom real final

        if (capo === 0) {
            formaLabel.textContent = `Mesmo tom`;
        } else {
            formaLabel.textContent = `Forma: ${formaKey}${quality}`;
        }

        // Sincroniza grade de tons (mostra o Tom Real na grade)
        document.querySelectorAll('#setlistKeyGrid .key-btn').forEach(b => {
            b.classList.toggle('active', b.dataset.key === realKey);
        });
    }
    ...
    function initSetlistGrids() {
        ...
        SCALE.forEach(note => {
            ...
            btn.onclick = () => {
                ...
                // Ao clicar num Tom, queremos que o TOM REAL seja esse.
                // TomReal = Initial + Manual + Capo => Manual = TomReal - Initial - Capo
                const iIdx = SCALE.indexOf(normalizeKey(initialKey));
                const targetIdx = SCALE.indexOf(note);
                _setlistManualSemitones = (targetIdx - iIdx - capo + 24) % 12;
                if (_setlistManualSemitones > 6) _setlistManualSemitones -= 12;

                _updateSetlistKeyDisplay();
    ...
    function _fillScalingMenu() {
        ...
        // Tentar detectar do campo de obs ou do texto da cifra
        const searchIn = (song.obs || '') + ' ' + (song.cifra_text || '');
        
        // 1) Capo
        const cM = searchIn.match(/Capo\s*(\d+)/i) || searchIn.match(/Capotraste\s+na\s+(\d+)/i);
        if (cM) detCapo = parseInt(cM[1]);

        // 2) Tom
        let displayKey = detectFormaKey(song.cifra_text || '') || detectFormaKey(song.obs || '');
        ...
        // Sincroniza botões ativos do capo
