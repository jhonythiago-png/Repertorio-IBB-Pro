...
async function fetchCifraClubContent(url) {
    ...
    try {
        // Usando AllOrigins como Proxy para evitar erro de CORS
        ...
        // O Cifra Club costuma guardar a cifra dentro de um <pre>
        const cifraElement = doc.querySelector('.cifra_cnt pre') || doc.querySelector('pre');
        
        if (cifraElement) {
            // Limpeza básica: Remover tags HTML se houver (o CC usa <b> para acordes)
            // Mas queremos manter os acordes! O doc.querySelector('.cifra_cnt pre').innerText costuma vir limpo.
            return cifraElement.innerText;
        } else {
            throw new Error('Não foi possível localizar o conteúdo da cifra na página.');
        }
    } catch (error) {
        console.error('Erro na captura:', error);
        alert('Falha ao capturar cifra: ' + error.message);
        return null;
    }
}
