...
function openPerformance(song, mode) {
    ...
    // Tenta identificar o tom original no texto
    const originalKeyMatch = (song.cifra_text || '').match(/Tom:\s*([A-G][b#]?)/i);
    const originalKey = originalKeyMatch ? originalKeyMatch[1] : 'Original';
...
function renderPerformanceContent() {
    ...
    // Aplicar Transposição se for músico
    if (currentPerformanceMode === 'musician' && currentTransposeSemitones !== 0) {
    ...
    lines.forEach(line => {
        ...
        // 1. Preservar espaços entre estrofes (apenas se for uma linha realmente vazia)
        if (trimmed === "") {
            const emptyDiv = document.createElement('div');
            emptyDiv.className = 'perf-line empty-line';
            emptyDiv.innerHTML = '&nbsp;'; // Garante que a linha tenha altura
            content.appendChild(emptyDiv);
            return;
        }

        // 2. Identificar se a linha é INSTRUMENTAL (Cifras, Tabs, Marcações)
        // Regex para Acordes: C, Am7, G/B, etc.
        const chordRegex = /^(?:[A-G][b#]?(?:m|maj|dim|aug|sus|add|[0-9]|M|\+|\-|\^|∆|º|ø)*|[\/\(\)\|\s\-\[\]\:\*áéíóúâêîôûàèìòùãõç]+)+$/i;
        
        // Regex para Tablaturas: E|---, B|---, etc.
        const isTabLine = /^[A-Ga-g]?\|[\-\d\s\|pbrh\/\(\)\~\>\.]+$/.test(trimmed);
        
        // Regex para Marcações Técnicas: [Solo], [Intro], Parte 1, etc.
        const isTechnical = /^[\[\(]?(?:Intro|Solo|Ponte|Refrão|Coro|Final|Tab|Parte|Dedilhado|Instrumental|Ponte|Verso)/i.test(trimmed);

        const isInstrumental = isChordLineByHeuristic(line) || isTabLine || isTechnical;

        if (currentPerformanceMode === 'vocal' && isInstrumental) {
            return; // No modo vocal, ignoramos tudo que não for letra
        }

        const lineDiv = document.createElement('div');
        lineDiv.className = 'perf-line';
        if (isInstrumental) lineDiv.classList.add('chord-line');
        
        lineDiv.textContent = line || ' ';
        content.appendChild(lineDiv);
    });

    content.style.fontSize = currentPerformanceMode === 'vocal' ? '' : `${currentZoomLevel}rem`;
}
...
document.querySelectorAll('.key-btn').forEach(btn => {
    btn.onclick = () => {
        ...
        if (originalKey === 'Original') {
            // Se não sabemos o tom original, a transposição fica relativa (não recomendado)
            alert('Tom original não detectado no texto. A transposição pode não ser exata.');
            return;
        }
...
function isChordLineByHeuristic(line) {
    ...
    words.forEach(word => {
        ...
        if (/^[A-G][b#]?(?:m|maj|dim|aug|sus|add|[0-9]|M|\+|\-|\/)*$/.test(word)) {
            potentialChords++;
        }
    });

    // Se mais de 50% das "palavras" da linha parecem acordes, removemos no modo Vocal
...
function updateTransposeDisplay() {
    ...
    const val = currentTransposeSemitones;
    keyDisplay.textContent = val === 0 ? 'Original' : (val > 0 ? `+${val}` : val);
}
    ...
    try {
        ...
        data.forEach(setItem => {
            ...
            if(originalSong) {
                ...
                const fakeSong = { title: setItem.song_title, artist: 'Excluída da base', obs: setItem.obs, vid_id: '' };
                const card = createSongCard(fakeSong);
                card.classList.add('setlist-card');
                grid.appendChild(card);
            }
        });
