...
function openPerformance(song, mode) {
    ...
    // Botão Próximo: visível se há próxima música na escala
    const nextBtn = document.getElementById('nextPerfBtn');
    if (nextBtn) {
        const hasNext = _setlistIndex >= 0 && (_setlistIndex + 1) < _setlistSongs.length;
...
document.getElementById('nextPerfBtn').onclick = () => {
    ...
    const nextIndex = _setlistIndex + 1;
    if (nextIndex < _setlistSongs.length) {
        _setlistIndex = nextIndex;
        openPerformance(_setlistSongs[nextIndex], currentPerformanceMode);
    }
};
...
async function loadSetlist(teamName) {
    ...
    try {
        ...
        // Guarda lista para navegação Próximo na performance
