...
function openPerformance(song, mode) {
    ...
    // Helper: detecta a primeira linha 100% de acordes (ignora linha [Intro] etc)
    function detectFormaKey(txt) {
        ...
        for (const line of lines) {
            ...
            // Todos os tokens devem ser acordes válidos (sem barra complexa)
            const allChords = tokens.every(tk =>
                /^[A-G][b#]?(?:m|maj|7|9|11|13|dim|aug|sus|add|M|\+|-)*(?:\/[A-G][b#]?)?$/.test(tk)
            );
            if (allChords) {
                // Pega o acorde base (antes da barra)
    ...
    if (capoDisplay && capoGrid) {
        ...
        capoGrid.querySelectorAll('.capo-btn-item').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                // Transpõe as notas da cifra proporcionalmente à mudança de capo
                // Ex: original capo 3 → nova capo 2 = -1 semitom
                ...
                // Atualiza indicador de tom: formaKey + novaCasa semitones = tom real
                const fIdx = SCALE.indexOf(normalizeKey(formaKey));
                if (fIdx !== -1) {
                    const realKey = novaCasa === 0 ? formaKey : SCALE[(fIdx + novaCasa) % 12];
