...
function renderPerformanceContent() {
    ...
    lines.forEach(line => {
        ...
        // 1. Preservar espaços originais (Modo Músico e Vocal respeitam o Cifra Club)
        if (trimmed === "") {
        ...
        // Regex para Marcações Técnicas: [Solo], [Intro], Tab, etc.
        ...
        // Marcações de Estrutura (QUE DEVEM SER MANTIDAS NO VOCAL)
        const isStructureMarker = /^[\[\(]?(?:Refrão|Coro|Ponte|Final|Verso|Parte|Primeira|Segunda|Terceira|Quarta)/i.test(trimmed);

        // No modo vocal, filtramos cifras e o que for puramente técnico
        if (currentPerformanceMode === 'vocal') {
            if (isCifraOrTab || isTechnicalToRemove) {
                // Se for cifra, ou marcador técnico (sem ser marcador de estrutura), removemos
                if (!isStructureMarker) return;
            }
            // NO MODO VOCAL: Apenas limpamos os espaços nos cantos para ficar reto à esquerda
            displayLine = trimmed;
        }
        ...
        if (isCifraOrTab || !isStructureMarker && isTechnicalToRemove) {
