...
async function fetchCifraClubContent(url) {
    ...
    // Lista de Proxies para tentar contornar o CORS
    const proxies = [
    ...
    console.log('Iniciando captura automática para:', url);

    for (const getProxyUrl of proxies) {
        try {
            const proxyUrl = getProxyUrl(url);
            console.log('Tentando via:', proxyUrl);
            
            const response = await fetch(proxyUrl);
            if (!response.ok) continue;

            const data = await response.json().catch(() => null);
            // AllOrigins retorna em 'contents', outros proxies podem retornar direto ou como texto
            let html = data ? (data.contents || data) : null; 
            
            if (!html || typeof html !== 'string') {
                html = await response.text();
            }

            if (!html || html.length < 500) continue; // Página muito curta provavelmente é erro

            const parser = new DOMParser();
            const doc = parser.parseFromString(html, 'text/html');
            
            // O Cifra Club pode usar .cifra_cnt pre ou apenas pre
            let cifraElement = doc.querySelector('.cifra_cnt pre') || doc.querySelector('pre');
            
            if (cifraElement) {
                console.log('✅ Conteúdo extraído com sucesso!');
                return cifraElement.innerText;
            }
        } catch (err) {
            console.warn('Falha em um dos proxies:', err);
        }
    }

    // Se chegou aqui, todos falharam
    const isLocalFile = window.location.protocol === 'file:';
    let msg = 'Não conseguimos capturar a cifra automaticamente.';
    
    if (isLocalFile) {
        msg += '\n\n⚠️ DETECTADO: Você está testando o arquivo localmente. O navegador bloqueia acessos externos.\n\nSOLUÇÃO: O portal funcionará melhor quando estiver online no GitHub.';
    } else {
        msg += '\n\nO Cifra Club bloqueou o acesso automático. \n\nSOLUÇÃO: Copie a letra no site deles e cole manualmente na caixa de texto abaixo.';
    }
...
function renderPerformanceContent() {
    ...
    lines.forEach(line => {
        ...
        // Identificar se a linha é só de acordes (Músico)
        const isChordLine = /^[A-G][b#]?(?:m|maj|dim|aug|sus|add|[0-9]| )*(?:\/[A-G][b#]?)? *$/.test(line);
        
        if (isChordLine) {
            if (currentPerformanceMode === 'vocal') {
                return; // Pula linha de cifra se for vocal
            }
    ...
    content.style.fontSize = `${currentZoomLevel}rem`;
}
