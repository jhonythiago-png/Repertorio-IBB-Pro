...
function renderPerformanceContent() {
    ...
    lines.forEach(line => {
        ...
        if (trimmed === "" && currentPerformanceMode === 'vocal') return; // Evita excesso de espaços no vocal

        // REGEX EXPERT: Identifica se a linha é composta predominantemente por acordes
        // Captura: C, Am7, G#m, D/F#, Bsus4, C(9), etc.
        const chordRegex = /^(?:[A-G][b#]?(?:m|maj|dim|aug|sus|add|[0-9]|M|\+|\-|\^|∆|º|ø)*|[\/\(\)\|\s\-\[\]\:]+)+$/;
        const isChordLine = chordRegex.test(line) && trimmed.length > 0;
        ...
        if (isChordLine) {
            ...
            lineDiv.className = 'perf-line chord-line';
