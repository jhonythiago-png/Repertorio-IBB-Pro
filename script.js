...
function openPerformance(song, mode) {
    ...
    if (tomLine) {
        ...
        // Pega primeiro acorde da cifra, mesmo em linhas com marcadores [Intro] etc
        const lines = textContent.split('\n');
        for (const line of lines) {
            const t = line.trim();
            if (!t || /^(Tom|Capotraste|Parte)/i.test(t)) continue;
            // Remove marcadores [Intro], [Verso] etc antes de procurar acordes
    ...
    if (capoDisplay && capoGrid) {
        ...
        capoGrid.querySelectorAll('.capo-btn-item').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                // formaKey = tom como escrito na cifra (ex: "G"), base para calcular tom real
                const formaKey = keyEl.dataset.formaKey || keyEl.dataset.original || null;
                ...
                // Atualiza indicador de tom: tom real = formaKey + novaCasa semitones
                if (formaKey && formaKey !== 'Original') {
                    const idx = SCALE.indexOf(normalizeKey(formaKey));
                    if (idx !== -1) {
                        const realKey = novaCasa === 0 ? formaKey : SCALE[(idx + novaCasa) % 12];
                        keyEl.textContent = realKey;
                    }
                }
                // Notas da cifra não mudam — só o indicador de tom
            });
        });
    }
