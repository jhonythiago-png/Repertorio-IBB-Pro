...
function renderPerformanceContent() {
    ...
    lines.forEach(line => {
        ...
        // Identifica o que é instrumental (Cifras ou Tabs)
        const isCifraOrTab = isChordLineByHeuristic(line) || isTabLine;
        
        // No modo vocal, ignoramos acordes e tabs, mas MANTEMOS as marcações técnicas (Intro, Solo, etc.)
        if (currentPerformanceMode === 'vocal' && isCifraOrTab) {
            return; 
        }
        ...
        // Cores em amarelo para cifras, tabs ou títulos técnicos
        const isHighlighted = isCifraOrTab || isTechnical;
        if (isHighlighted) lineDiv.classList.add('chord-line');
