...
function openPerformance(song, mode) {
    ...
    const keyContainer = document.querySelector('.control-group:has(#perfKeyDisplay)');
    
    if (mode === 'vocal') {
        screen.classList.add('vocal-mode');
        if (keyContainer) keyContainer.style.display = 'none'; // Esconde tom no vocal
    } else {
        if (keyContainer) keyContainer.style.display = 'flex'; // Mostra tom nos músicos
    }
...
function renderPerformanceContent() {
    ...
    lines.forEach(line => {
        ...
        // 1. Preservar espaços entre estrofes
        if (trimmed === "") {
            // No modo vocal, ignoramos linhas vazias internas para evitar gaps
            if (currentPerformanceMode === 'vocal') return;
        ...
        // Regex para Marcações Técnicas: [Solo], [Intro], Parte 1, Tab - Solo, etc.
        const isTechnical = /^[\[\(]?(?:Intro|Solo|Ponte|Refrão|Coro|Final|Tab|Parte|Dedilhado|Instrumental|Verso)/i.test(trimmed) || 
                            trimmed.toLowerCase().startsWith('parte ') || 
                            trimmed.toLowerCase().includes('tab -');

        // No modo vocal, ignoramos acordes, tabs E marcações instrumentais (Intro, Solo, Partes de Solo)
        if (currentPerformanceMode === 'vocal') {
            // Removemos cifras, tabs e marcações de solo
            if (isCifraOrTab || isTechnical) {
                // EXCEÇÃO: Mantemos Refrão e Coro se o cantor quiser, mas nos prints o usuário marcou para remover "Parte X de Y" e "Tab - Solo"
                // Vou remover tudo que for técnico para deixar o vocal 100% limpo
                return;
            }
            // NO MODO VOCAL: Forçamos o trim para evitar indentações irregulares
            displayLine = trimmed;
        }
        ...
        // Cores em amarelo para cifras, tabs ou títulos técnicos (Apenas no modo Músico)
        if (isCifraOrTab || isTechnical) {
