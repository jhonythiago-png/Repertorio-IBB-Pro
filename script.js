...
window.addEventListener('DOMContentLoaded', () => {
    ...
    function _fillScalingMenu() {
        ...
        if (song.cifra_text) {
            ...
            // 2) Tom (Usa a mesma lógica da tela de Performance para consistência)
            let displayKey = null;
            ...
            // Fallback para detecção por acordes se não houver linha de Tom
            if (!displayKey) {
                displayKey = typeof detectFormaKey === 'function' ? detectFormaKey(txt) : null;
                // Se detectou por forma e tem capo, ajusta para o tom real
        ...
        // 3) Popula o Select
        tomSelect.innerHTML = '';
        const targetNorm = normalizeKey(detKey).trim();

        SCALE.forEach(note => {
            const displayNote = note + detQual;
            const opt = document.createElement('option');
            opt.value = displayNote;
            opt.textContent = displayNote;
            if (normalizeKey(note).trim() === targetNorm) opt.selected = true;
            tomSelect.appendChild(opt);
        });

        if (capoSelect) capoSelect.value = String(detCapo);
    }
    ...
    if (addToSetlistBtn) {
        addToSetlistBtn.onclick = () => {
            ...
            // Simples toggle: o preenchimento já foi feito no openVideo
