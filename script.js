...
if(addToSetlistBtn) {
    addToSetlistBtn.onclick = () => {
        ...
        // Preenche tom e capo com o original da música atual
        if (!isVisible) {
            _populateSetlistTomCapo();
        }
    };
}
...
function _populateSetlistTomCapo() {
    ...
    // Popula o select de tom com todas as notas, pré-selecionando o original
    tomSelect.innerHTML = '<option value="">Original da cifra</option>';
    SCALE.forEach(note => {
        const opt = document.createElement('option');
        opt.value = note;
        opt.textContent = note;
        opt.selected = (note === detectedKey);
