...
function openPerformance(song, mode) {
    ...
    // ── Tom completo: "Bb (forma dos acordes no tom de G)" ──────────
    const tomFullMatch = textContent.match(/Tom:\s*([A-G][b#]?(?:\s*\([^)]+\))?)/i);
    const tomSubLabel  = document.getElementById('perfTomSubLabel');

    if (tomSubLabel) {
        if (tomFullMatch && mode === 'musician') {
            const full       = tomFullMatch[1].trim();
            // Extrai só o conteúdo dos parênteses ex: "(forma G)" -> "forma G"
            const parenMatch = full.match(/\(([^)]+)\)/);
            if (parenMatch) {
                tomSubLabel.textContent  = parenMatch[1]; // "forma G"
    ...
    if (song.vid_id) {
        ...
        if (_playerToggle)  _playerToggle.style.display = 'none';
    }
    // ─────────────────────────────────────────────────────────────────
...
async function loadSetlist(teamName) {
    ...
    try {
        ...
        data.forEach((setItem, idx) => {
            ...
            // ADM: drag-to-reorder + botão remover
            if (isAdmin) {
                ...
                // Botão remover música da escala (só ADM)
                ...
                removeBtn.onclick = async (e) => {
                    ...
                    try {
                        ...
                        card.remove();
                        // Renumerar badges
                        document.querySelectorAll('#teamSetlistGrid .setlist-pos-badge')
