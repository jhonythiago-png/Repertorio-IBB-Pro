...
function openPerformance(song, mode) {
    ...
    function detectFormaKey(txt) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const allChords = tokens.every(tk =>
                /^[A-G][b#]?(?:m|maj|7|9|11|13|dim|aug|sus|add|M|\+|-)*(?:\/[A-G][b#]?)?$/.test(tk)
            );
    ...
    keyIndicator.dataset.original   = formaKey;   // base para transposição (notas da cifra)
    keyIndicator.dataset.displayKey = displayKey; // tom real para restore (ex: Bb)
    currentTransposeSemitones = 0;
    ...
    if (capoDisplay) {
        ...
        if (capoText) capoText.textContent = capoOriginal === 0 ? 'Sem capo' : `${capoOriginal}ª casa`;
...
document.querySelectorAll('.key-btn').forEach(btn => {
    btn.onclick = () => {
        ...
        // Salva como semitones manuais para o capo poder combinar corretamente
        const cd = document.getElementById('perfCapoDisplay');
        if (cd) cd.dataset.manualSemitones = currentTransposeSemitones;
...
document.getElementById('restoreKey').onclick = () => {
    ...
    if (capoDisplay && capoText) {
        ...
        capoText.textContent = capoOrig === 0 ? 'Sem capo' : `${capoOrig}ª casa`;
        document.querySelectorAll('.capo-btn-item').forEach(b => {
...
function updateKeyIndicatorFromSemitones() {
    const keyIndicator = document.getElementById('perfKeyDisplay');
    // Base = formaKey (notas como escritas na cifra, ex: G)
    const baseKey    = keyIndicator.dataset.original || 'C';
    const startIndex = SCALE.indexOf(normalizeKey(baseKey));
...
function isChordLineByHeuristic(line) {
    ...
    let chordCount = 0;
    words.forEach(word => {
        // Acorde válido: A-G + modificadores opcionais + barra opcional
        if (/^[A-G][b#]?(?:m|maj|7|9|11|13|dim|aug|sus|add|M|\+|-)*(?:\/[A-G][b#]?)?$/.test(word)) {
            chordCount++;
        }
    });
    ...
    if (capoDisplay && capoGrid) {
        ...
        capoGrid.querySelectorAll('.capo-btn-item').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                // Transposição manual atual (stepUp/Down ou key-btn)
                const manualSemitones = parseInt(capoDisplay.dataset.manualSemitones) || 0;

                // Capo contribui com semitones adicionais em relação ao original
                const capoDiff = novaCasa - casaOriginal;

                // Total = manual + capo diff
                currentTransposeSemitones = ((manualSemitones + capoDiff) % 12 + 12) % 12;
                ...
                // Tom real = formaKey + total semitones
                const fIdx = SCALE.indexOf(normalizeKey(formaKey));
                if (fIdx !== -1) {
                    keyEl.textContent = SCALE[(fIdx + currentTransposeSemitones) % 12];
                }
