    ...
    function detectFormaKey(txt) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const allChords = tokens.every(tk =>
                /^[A-G][b#]?(?:m|maj|dim|aug|sus|add)?(?:\d{1,2})?(?:M|maj)?(?:\([\d\/]+\))?(?:\/[A-G][b#]?(?:\d)?)?$/.test(tk)
            );
...
function isChordLineByHeuristic(line) {
    ...
    let chordCount = 0;
    words.forEach(word => {
        // Regex: suporta Gm/F, Eb7M, F7(4/9), Bb7M(9), D4/G, Bb4, etc.
        if (/^[A-G][b#]?(?:m|maj|dim|aug|sus|add)?(?:\d{1,2})?(?:M|maj)?(?:\([\d\/]+\))?(?:\/[A-G][b#]?(?:\d)?)?$/.test(word)) {
            chordCount++;
        }
    });
