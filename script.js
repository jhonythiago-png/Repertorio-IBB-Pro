...
function isChordLineByHeuristic(line) {
    ...
    // Remove parênteses externos: "( D11  Em  D/F# )" → "D11  Em  D/F#"
    const stripped = trimmed.replace(/^\(\s*/, '').replace(/\s*\)$/, '').trim();
    ...
    let chordCount = 0;
    words.forEach(word => {
        // Regex expandido para suportar:
        // - Extensões numéricas: 4, 5, 6, 7, 9, 11, 13
        // - Modificadores entre parênteses: F7(4/9), Bb7M(9)
        // - Sufixos compostos: dim, aug, sus, add, maj, M, m
        // - Barra de baixo: C/E, Bb/F, D4/G
        if (/^[A-G][b#]?(?:m|maj|M|dim|aug|sus|add)?(?:\d{1,2})?(?:\([\d/]+\))?(?:\/[A-G][b#]?(?:\d)?)?$/.test(word)) {
            chordCount++;
        }
    });
