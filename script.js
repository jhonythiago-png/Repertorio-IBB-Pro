...
function openPerformance(song, mode) {
    ...
    if (mode === 'vocal') {
        ...
        if (keyContainer) keyContainer.style.display = 'flex'; // Volta a mostrar no músico
    }
...
function renderPerformanceContent() {
    ...
    lines.forEach(line => {
        ...
        if (trimmed === "") {
            // No modo vocal, só permitimos UM espaço se já houver conteúdo antes
            if (currentPerformanceMode === 'vocal') {
                if (!addedAnyContent || lastWasEmpty) return;
                lastWasEmpty = true;
        ...
        if (currentPerformanceMode === 'vocal') {
            // Removemos cifras e itens puramente instrumentais
            if (isCifraOrTab) return;
            
            // Removemos marcações de Intro e Solo, mas MANTEMOS Refrão, Verso e Parte X para guiar o cantor
            if (isTechnical) {
                const isHeavyTechnical = /^[\[\(]?(?:Intro|Solo|Tab|Dedilhado|Instrumental)/i.test(trimmed) || trimmed.toLowerCase().includes('tab -');
                if (isHeavyTechnical) return;
            }
