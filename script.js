...
// ─── Padronização de Texto (Título e Artista) ────────────────────────
function formatStandardText(text) {
    if (!text) return "";
    const exceptions = ["de", "do", "da", "e", "com", "em", "para", "dos", "das"];
    return text.trim().toLowerCase().split(/\s+/).map((word, index) => {
        if (word === "ibb") return "IBB";
...
async function handlePasswordSubmit() {
    ...
    if (team) {
        ...
        // Tracking Login
        trackAppEvent('leader_login', { team: team });
...
function openPerformance(song, mode) {
    ...
    title.textContent = formatStandardText(song.title);
    artist.textContent = formatStandardText(song.artist || 'Artista Desconhecido');
    
    // Tracking Song View
    trackAppEvent('view_song', {
        song_title: song.title,
        song_artist: song.artist || 'Desconhecido',
        view_mode: mode,
        team: getLoggedTeam()
    });
...
function renderPerformanceContent() {
    ...
    if (isVocal) {
        ...
        lines.forEach(line => {
            ...
            // CORREÇÃO: No vocal, se for linha de cifra/tab, removemos SEMPRE, 
            // mesmo que tenha marcador [Final], para não poluir a letra.
            if (isCifraOrTab) return;
...
function createSongCard(song) {
    ...
    card.innerHTML = `
        ...
        <div class="card-info">
            <h4>${formatStandardText(song.title)}</h4>
            <p>${formatStandardText(song.artist || 'Vários')} ${song.status ? '• ' + song.status : ''}</p>
        </div>
    `;
    card.onclick = () => {
...
function openVideo(song) {
    ...
    if (mTitle) mTitle.textContent = formatStandardText(song.title);
    if (mArtist) mArtist.textContent = formatStandardText(song.artist || 'Vários');
...
async function loadSetlist(teamName) {
    ...
    try {
        ...
        _setlistSongs = data.map(setItem => {
            const searchTitle = (setItem.song_title || '').toLowerCase().trim();
            let found = null;

            // 1ª Tentativa: Busca GLOBAL por ID em todas as categorias
            if (setItem.song_id) {
                for (const cat of repertoireData) {
                    found = cat.items.find(s => s.id === setItem.song_id);
                    if (found) break;
                }
            }

            // 2ª Tentativa: Se não achou por ID (ou não tinha ID), busca por título
            if (!found) {
                for (const cat of repertoireData) {
                    found = cat.items.find(s => (s.title || '').toLowerCase().trim() === searchTitle);
                    if (found) break;
                }
            }
        ...
        data.forEach((setItem, idx) => {
            ...
            // 1ª Tentativa: Busca GLOBAL por ID
            if (setItem.song_id) {
                for (const cat of repertoireData) {
                    const found = cat.items.find(s => s.id === setItem.song_id);
                    if (found) { originalSong = found; break; }
                }
            }

            // 2ª Tentativa: Busca por Título
            if (!originalSong) {
                for (const cat of repertoireData) {
                    const found = cat.items.find(s => (s.title || '').toLowerCase().trim() === searchTitle);
                    if (found) { originalSong = found; break; }
                }
            }
            ...
            const songForCard = originalSong
                ...
                : { title: formatStandardText(setItem.song_title), artist: 'Excluída da base', obs: setItem.obs, url: '' };
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (teamParam) {
        ...
        const autoOpenInterval = setInterval(() => {
            ...
            // Esperar o seletor ter opções (populado pelo script) e o Nav estar disponível
            if (TS && TS.options.length > 0 && teamsNav) {
            ...
            if (attempts > 20) clearInterval(autoOpenInterval); // Para após 10 segundos
        }, 500);
    }
    ...
    if (saveSetlistBtn) {
        saveSetlistBtn.onclick = async () => {
            ...
            const insertData = { team, song_title: songTitle, obs, tom_key: tomKey, capo_fret: capoFret };
            
            // Tentativa inteligente: incluir song_id se disponível para resolver duplicatas de título
            if (window._modalCurrentSong && window._modalCurrentSong.id) {
                insertData.song_id = window._modalCurrentSong.id;
            }

            try {
                let { error } = await _supabase.from('setlists').insert([insertData]);
                
                // Fallback: se der erro de coluna inexistente, tenta sem o song_id
                if (error && (
                    error.message.includes('song_id') || 
                    error.message.includes('schema cache') || 
                    error.code === 'PGRST204' || 
                    error.code === '42703'
                )) {
                    console.warn("Coluna 'song_id' não encontrada. Usando fallback.");
                    delete insertData.song_id;
                    const retry = await _supabase.from('setlists').insert([insertData]);
                    error = retry.error;
                }
    ...
    if (shareSetlistBtn) {
        shareSetlistBtn.onclick = () => {
            ...
            // Ícones ultra-compatíveis
            const iconMusic   = decodeURIComponent('%F0%9F%83%B5'); // 🎵 (Ajustado para nota musical)
            const iconMic     = decodeURIComponent('%F0%9F%8E%A4'); // 🎤
            const iconFolder  = decodeURIComponent('%F0%9F%93%82'); // 📂
            const iconPiano   = decodeURIComponent('%F0%9F%8E%B9'); // 🎹
            const iconGuitar  = decodeURIComponent('%F0%9F%8E%B8'); // 🎸
            const iconObs     = decodeURIComponent('%F0%9F%93%9D'); // 📝
            const iconVideo   = decodeURIComponent('%F0%9F%8E%A5'); // 🎥
            const iconLink    = decodeURIComponent('%F0%9F%93%B2'); // 📲

            let message = `━━━━━━━━━━━━━━━━━━━━━━\n`;
            message += `  \u{1F3B5} *SETLIST*\n`;
            message += `━━━━━━━━━━━━━━━━━━━━━━\n\n`;

            _setlistSongs.forEach((song, idx) => {
                message += `*${idx + 1}. ${formatStandardText(song.title)}*\n`;
                
                if (song.artist) {
                    message += `${iconMic} ${formatStandardText(song.artist)}\n`;
                }
                ...
                if (song.vid_id) {
                    // Sem protocolo para evitar prévia múltipla
                    message += `${iconVideo} Vídeo: youtu.be/${song.vid_id}\n`;
                }

                message += `\n`;
            });

            // Link principal com protocolo para ativar a prévia profissional no WhatsApp
