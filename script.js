    ...
    try {
        ...
        _setlistSongs = data.map(setItem => {
            for (const cat of repertoireData) {
                // Prioriza busca por ID se disponível, para evitar erro com títulos duplicados
                let found = setItem.song_id ? cat.items.find(s => s.id === setItem.song_id) : null;
                if (!found) found = cat.items.find(s => s.title === setItem.song_title);
    ...
    if (saveSetlistBtn) {
        saveSetlistBtn.onclick = async () => {
            ...
            try {
                ...
                if (error && (error.message.includes('song_id') || error.code === 'PGRST204' || error.code === '42703')) {
                    console.warn("Coluna 'song_id' não encontrada na tabela setlists. Usando fallback por título.");
