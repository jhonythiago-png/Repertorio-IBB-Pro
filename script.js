...
function openPerformance(song, mode) {
    ...
    // Detecta tom original: prioriza linha "Tom: X" do cifra_text
    let originalKey = 'Original';
    const textContent = song.cifra_text || '';
    const originalKeyMatch = textContent.match(/^Tom:\s*([A-G][b#]?)/im);
    
    if (originalKeyMatch) {
        originalKey = originalKeyMatch[1];
    }
    // Sem linha Tom: no texto → mantém 'Original' (sem adivinhar errado)
    ...
    if (capoMatch && capoGroup && capoDisplay) {
        ...
        capoDisplay.dataset.capoOriginal = _currentCapo; // casa original da cifra
        capoDisplay.dataset.baseTranspose = currentTransposeSemitones; // transposição manual atual
        const capoText = document.getElementById('perfCapoText');
        if (capoText) capoText.textContent = `${_currentCapo}ª casa`;
        // Capotraste só aparece no modo músico
        ...
        if (capoDisplay) { capoDisplay.dataset.capo = 0; capoDisplay.dataset.capoOriginal = 0; capoDisplay.dataset.baseTranspose = 0; }
    }
    ...
    if (tomFullMatch && tomSubLabel) {
        const full = tomFullMatch[1].trim();
        // Separa "Bb" de "(forma G)" se houver parênteses
        const parenMatch = full.match(/^([A-G][b#]?)\s*(\(.+\))?$/);
        if (parenMatch && parenMatch[2]) {
            tomSubLabel.textContent = parenMatch[2]; // "(forma G)"
            tomSubLabel.style.display = 'block';
        } else {
            tomSubLabel.style.display = 'none';
        }
        keyIndicator.title = 'Tom: ' + full;
    } else if (tomSubLabel) {
        tomSubLabel.style.display = 'none';
    }
    // ─────────────────────────────────────────────────────────────────
    ...
    if (capoDisplay && capoGrid) {
        ...
        capoGrid.querySelectorAll('.capo-btn-item').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                const casaAnterior = parseInt(capoDisplay.dataset.capo) || 0;
                const casaOriginal = parseInt(capoDisplay.dataset.capoOriginal) || 0;

                // Diferença de semitones em relação ao capo original da cifra
                const capoDiff = novaCasa - casaOriginal;
                
                // Transposição total = transposição manual do tom + ajuste de capo
                const baseTranspose = parseInt(capoDisplay.dataset.baseTranspose) || 0;
                currentTransposeSemitones = (baseTranspose + capoDiff + 12) % 12;
                ...
                // Atualiza o indicador de tom na UI
                updateKeyIndicatorFromSemitones();
                renderPerformanceContent();
            });
        });
