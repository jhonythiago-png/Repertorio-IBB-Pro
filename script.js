...
// â”€â”€â”€ Controle de Acesso de LÃ­deres â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
if (admBtn) {
    admBtn.onclick = () => {
        if (isAuthenticated()) {
            if (confirm('Sair do modo de lideranÃ§a?')) {
                setAdminMode(false);
            }
            return;
        }
        ...
        const handleSubmit = async () => {
            ...
            if (TEAM_PASSWORDS[pwd]) {
                ...
                // Refresh atual visualizaÃ§Ã£o se tiver na aba de equipes
...
// â”€â”€â”€ MODO LOUVOR STATE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const praiseState = {
    song: null,
...
// Escala cromÃ¡tica (preferÃªncia por sustenidos)
...
// â”€â”€â”€ Fetch da Cifra Club via CORS Proxy â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function fetchCifraFromCifraClub(url) {
    const cleanUrl = url.split('#')[0].split('?')[0].replace(/\/$/, '') + '/';
    const proxyUrl = `https://api.allorigins.win/get?url=${encodeURIComponent(cleanUrl)}`;
    const resp = await fetch(proxyUrl);
    if (!resp.ok) throw new Error('Proxy indisponÃ­vel (status ' + resp.status + ')');
    const json = await resp.json();
    if (!json.contents) throw new Error('Nenhum conteÃºdo retornado pelo proxy');
...
function parseCifraClubHtml(html) {
    ...
    // Seletores possÃ­veis para o bloco de cifra
    const selectors = [
...
// â”€â”€â”€ Abertura / Fechamento do Modo Louvor â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function openPraiseMode(song, mode = 'vocal') {
    ...
    document.getElementById('praiseSongArtist').textContent = song.artist || 'VÃ¡rios';
        ...
        if (!lyrics.trim()) {
            contentEl.innerHTML = `
                <div class="praise-no-cifra">
                    ...
                    <p>Letra nÃ£o cadastrada para esta mÃºsica.</p>
                    <p style="font-size:0.82rem;color:#444;margin-top:8px;">Um ADM pode adicionar a letra nas configuraÃ§Ãµes da mÃºsica.</p>
                </div>`;
            return;
        }
        ...
        if (!cifraText || !cifraText.trim()) {
            contentEl.innerHTML = `
                <div class="praise-no-cifra">
                    ...
                    <p>Cifra nÃ£o cadastrada para esta mÃºsica.</p>
                    <p style="font-size:0.82rem;color:#444;margin-top:8px;">Um ADM pode adicionar o link do Cifra Club nas ediÃ§Ãµes da mÃºsica.</p>
                </div>`;
            return;
        }
        // TransposiÃ§Ã£o: semitones escolhidos pelo usuÃ¡rio, menos capo para facilitar o mÃºsico
...
// â”€â”€â”€ Bind de todos os controles do Modo Louvor (chamado apÃ³s DOM pronto) â”€â”€â”€
function setupPraiseModeControls() {
    document.getElementById('praiseModeClose').onclick = closePraiseMode;

    // Toggle Vocal / MÃºsico
...
const FIXED_CATEGORIES = [
    "Novos",
    "Convite", 
    "CelebraÃ§Ã£o/AdoraÃ§Ã£o/Louvor", 
    "ConsagraÃ§Ã£o", 
    "Busca", 
    "ContemplaÃ§Ã£o/AdoraÃ§Ã£o e Louvor", 
    "Ceia", 
    "ComunhÃ£o", 
    "FÃ©", 
    "ClÃ¡ssicas"
...
async function fetchData() {
    try {
        ...
        // Montar estrutura final apenas com categorias que tÃªm mÃºsicas
        ...
        updateFooter();
        initAdminState(); // Respeita estado de auth entre navegaÃ§Ãµes
    } catch (error) {
...
function createSongCard(song) {
    ...
    card.innerHTML = `
        ...
        <div class="card-info">
            <h4>${song.title}</h4>
            <p>${song.artist || 'VÃ¡rios'} ${song.status ? 'â€¢ ' + song.status : ''}</p>
        </div>
    `;
...
function openVideo(song) {
    ...
    if (cifraStatus) cifraStatus.textContent = song.cifra_text ? 'âœ… Cifra disponÃ­vel nesta mÃºsica.' : 'âš ï¸ Nenhuma cifra cadastrada ainda.';

    // Resetar zona de exclusÃ£o
    ...
    // â”€â”€ Preencher checkboxes de categorias â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const editCatGrid = document.getElementById('editCategoryCheckboxes');
    ...
    // â”€â”€ BotÃ£o Editar VersÃ£o (toggle painel) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    const editBtn = document.getElementById('editUrlBtn');
    editBtn.onclick = () => {
    ...
    // â”€â”€ Fechar painel â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    document.getElementById('cancelUrlBtn').onclick = () => {
        urlEditorPanel.style.display = 'none';
    };

    // â”€â”€ Salvar URL â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    document.getElementById('saveUrlBtn').onclick = async () => {
        const newUrl = newUrlInput.value.trim();
        if (!newUrl) { alert('Cole uma URL vÃ¡lida!'); return; }
    ...
    // â”€â”€ Salvar Nome do Louvor â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    document.getElementById('saveTitleBtn').onclick = async () => {
        const newTitle = newTitleInput.value.trim();
        if (!newTitle) { alert('Digite o nome do louvor!'); return; }
        if (newTitle === song.title) { alert('O nome nÃ£o foi alterado.'); return; }
        ...
        try {
            ...
            alert('âœ… Nome do louvor atualizado!');
            fetchData();
        } catch (err) {
            btn.innerHTML = '<i class="fas fa-save"></i> Salvar Nome';
            alert('Erro: ' + err.message);
    ...
    // â”€â”€ Salvar Artista â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Salvar ObservaÃ§Ãµes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    document.getElementById('saveObsBtn').onclick = async () => {
        ...
        try {
            ...
            alert('âœ… ObservaÃ§Ã£o atualizada!');
            fetchData();
        } catch (err) {
            btn.innerHTML = '<i class="fas fa-save"></i> Salvar Obs';
            alert('Erro: ' + err.message);
    ...
    // â”€â”€ Salvar Letra â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Salvar Cifra Club URL + Buscar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    document.getElementById('saveCifraBtn').onclick = async () => {
        ...
        if (cifraStatusEl) cifraStatusEl.textContent = 'ðŸ”„ Tentando buscar no Cifra Club...';
        try {
            // Salva URL
            ...
            if (cifraText) {
                ...
                if (cifraStatusEl) cifraStatusEl.textContent = 'âœ… Cifra buscada e salva com sucesso!';
                alert('âœ… Cifra do Cifra Club salva!');
            } else {
                if (cifraStatusEl) cifraStatusEl.textContent = 'âš ï¸ URL salva, mas a cifra nÃ£o pÃ´de ser extraÃ­da automaticamente. Tente novamente mais tarde.';
            }
        } catch (err) {
            if (cifraStatusEl) cifraStatusEl.textContent = 'âŒ Erro: ' + err.message;
            alert('Erro ao buscar cifra: ' + err.message);
        }
        btn.innerHTML = '<i class="fas fa-download"></i> Salvar e Buscar Cifra';
    };

    // â”€â”€ Salvar Categorias â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
”€â”€â”€
    ...
    // â”€â”€ Excluir MÃºsica (2 etapas) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    document.getElementById('confirmDeleteBtn').onclick = async () => {
        ...
        console.log('Arquivando mÃºsica (Soft Delete):', { id: song.id, title: song.title });

        try {
            // Soft delete: Apenas marca como DELETED para o sincronizador respeitar a exclusÃ£o
            ...
            if (error) {
                console.error('Erro ao arquivar mÃºsica:', error);
                throw error;
            }

            console.log('MÃºsica arquivada:', data);

            modal.style.display = 'none';
            videoIframe.src = '';
            alert(`âœ… "${song.title}" foi removido do repertÃ³rio.`);
            
            // ForÃ§a a recarga total dos dados
            setTimeout(async () => {
                await fetchData();
            }, 300);
            
        } catch (err) {
            console.error('Erro fatal na exclusÃ£o:', err);
...
// Fechar menu mobile ao clicar em qualquer opÃ§Ã£o
...
searchInput.oninput = (e) => {
    ...
    // Garantir unicidade total por tÃ­tulo (trim para evitar espaÃ§os diferentes)
    const uniqueSongsMap = new Map();
    allSongs.forEach(s => {
        const titleLower = s.title.toLowerCase().trim();
        const artistLower = (s.artist || '').toLowerCase().trim();
        
        // Se estiver na blocklist e jÃ¡ houver uma versÃ£o limpa, ou for apenas indesejada, pula
...
document.getElementById('newNav').onclick = (e) => {
    ...
    renderGrid(songs, "Novos LanÃ§amentos");
};
    ...
    try {
        ...
        data.forEach(setItem => {
            ...
            const songForCard = originalSong
                ...
                : { title: setItem.song_title, artist: 'ExcluÃ­da da base', obs: setItem.obs, vid_id: '' };
            ...
            praiseBtns.innerHTML = `
                ...
                <button class="setlist-praise-btn musician-btn">
                    <i class="fas fa-guitar"></i> MÃºsico
                </button>`;
...
if(saveSetlistBtn) {
    saveSetlistBtn.onclick = async () => {
         ...
         try {
             ...
             alert(`Adicionado com sucesso Ã  escala do ${team}!`);
             document.getElementById('setlistEditorPanel').style.display = 'none';
             ...
             alert('Erro ao adicionar Ã  escala: ' + err.message);
         } finally {
...
if(clearSetlistBtn) {
    clearSetlistBtn.onclick = async () => {
        ...
        if(!confirm(`âš ï¸ Tem certeza que deseja LIMPAR TODA A ESCALA do ${team}?`)) return;
...
// Start
// --- SincronizaÃ§Ã£o AutomÃ¡tica e Real-time ---

// 1. Atualizar ao voltar para o app (Foco)
document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') {
        console.log("App visÃ­vel, atualizando dados...");
        fetchData();
        
        // Se estiver na aba de equipes, recarrega a escala tambÃ©m
        const teamsCont = document.getElementById('teamsContainer');
        const TS = document.getElementById('teamSelector');
...
setInterval(() => {
    console.log("Executando atualizaÃ§Ã£o periÃ³dica (2 min)...");
    fetchData();
...
// 3. Supabase Real-time (InscriÃ§Ã£o em mudanÃ§as no Banco)
const songsChannel = _supabase
    ...
    .on(
        ...
        (payload) => {
            console.log('MudanÃ§a detectada na tabela songs:', payload);
            fetchData();
        }
    )
    .on(
        ...
        (payload) => {
            console.log('MudanÃ§a detectada na tabela setlists:', payload);
            const teamsCont = document.getElementById('teamsContainer');
            const TS = document.getElementById('teamSelector');
...
// BotÃ£o de AtualizaÃ§Ã£o ForÃ§ada (Manual) - Ãštil para Standalone/App
document.getElementById('forceRefreshBtn').addEventListener('click', () => {
    const btn = document.getElementById('forceRefreshBtn');
    btn.classList.add('spinning');
    
    // Pequeno delay para mostrar a animaÃ§Ã£o antes de recarregar
    setTimeout(() => {
