...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (shareSetlistBtn) {
        shareSetlistBtn.onclick = () => {
            ...
            // Método ultra-robusto para emojis usando CodePoints (evita erros de codificação)
            const iconChurch = String.fromCodePoint(0x26EA);
            const iconTeam   = String.fromCodePoint(0x1F465);
            const iconMic    = String.fromCodePoint(0x1F3A4);
            const iconFolder = String.fromCodePoint(0x1F4C2);
            const iconPiano  = String.fromCodePoint(0x1F3B9);
            const iconGuitar = String.fromCodePoint(0x1F3B8);
            const iconObs    = String.fromCodePoint(0x1F4DD);
            const iconLink   = String.fromCodePoint(0x1F4F2);
            ...
            _setlistSongs.forEach((song, idx) => {
                // Título em negrito e numeração simples para maior compatibilidade
                ...
                // Tom e Capo na mesma linha se existirem
            ...
            const currentUrl = (window.location.protocol === 'file:') 
                ? 'https://repertorio-ibb.vercel.app' // Link exemplo ou do seu domínio
                : window.location.origin + window.location.pathname;

            message += `──────────────────────\n`;
            message += `${iconLink} *Acesse em:* ${currentUrl}\n`;
            message += `━━━━━━━━━━━━━━━━━━━━━━`;
