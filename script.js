...
function openPerformance(song, mode) {
    ...
    const tomLine      = tomLineMatch ? tomLineMatch[1].trim() : null; // "Bb (forma dos acordes no tom de G)"
    ...
    // 3) Determina o tom para exibir no botão
    //    Prioridade: linha Tom: → tom real (ex: "Bb")
    //    Fallback: tom da forma (primeiros acordes) + semitones do capo
    let displayKey   = 'Original'; // o que aparece no botão
    let formaKey     = null;       // tom da forma dos acordes (ex: "G")
    let subLabelText = null;       // texto abaixo do botão

    if (tomLine) {
        // Tem linha Tom: — extrai tom real e tom da forma
        ...
        // Sublabel: a parte entre parênteses
        const parenMatch = tomLine.match(/(\(.+\))/);
        if (parenMatch) subLabelText = parenMatch[1]; // "(forma dos acordes no tom de G)"
    } else if (capoOriginal > 0) {
        // Sem linha Tom: mas tem capo — calcula tom real a partir dos acordes + capo
        // Pega primeiro acorde puro da cifra
        const lines = textContent.split('\n');
        for (const line of lines) {
            const t = line.trim();
            if (!t || /^(Tom|Capotraste|Parte|Intro|Tab|Solo|\[)/i.test(t)) continue;
            const tokens = t.replace(/\[.*?\]/g, '').trim().split(/\s+/);
            const chord  = tokens.find(tk => /^[A-G][b#]?(?:m|maj|dim|aug|sus|add|[0-9])*$/.test(tk));
            if (chord) { formaKey = chord; break; }
        }
        if (formaKey) {
            // tom real = forma transposta pelo capo
    ...
    keyIndicator.dataset.original = formaKey || displayKey; // base para transposição = tom da forma
    keyIndicator.dataset.formaKey = formaKey || '';         // tom da forma (ex: "G") para cálculo do capo
    currentTransposeSemitones = 0; // reset ao abrir nova música
    ...
    // 4) Capotraste
    if (capoOriginal > 0 && capoGroup && capoDisplay) {
        capoDisplay.dataset.capo         = capoOriginal;
        capoDisplay.dataset.capoOriginal  = capoOriginal;
        capoDisplay.dataset.baseTranspose = 0;
        const capoText = document.getElementById('perfCapoText');
        if (capoText) capoText.textContent = `${capoOriginal}ª casa`;
        capoGroup.style.display = (mode === 'musician') ? 'flex' : 'none';
        document.querySelectorAll('.capo-btn-item').forEach(b => {
            b.classList.toggle('active', parseInt(b.dataset.capo) === capoOriginal);
        });
    } else if (capoGroup) {
        // Sem capo na cifra — mostra o botão mesmo assim no modo músico (começa em "Sem capo")
        if (capoDisplay) {
            capoDisplay.dataset.capo = 0;
            capoDisplay.dataset.capoOriginal = 0;
            capoDisplay.dataset.baseTranspose = 0;
            const capoText = document.getElementById('perfCapoText');
            if (capoText) capoText.textContent = 'Sem capo';
            document.querySelectorAll('.capo-btn-item').forEach(b => {
                b.classList.toggle('active', parseInt(b.dataset.capo) === 0);
            });
        }
        capoGroup.style.display = (mode === 'musician') ? 'flex' : 'none';
    }
    ...
    if (song.vid_id) {
        ...
        if (_playerWrapper) _playerWrapper.style.display = 'none'; // começa oculto
        if (_playerToggle)  {
            _playerToggle.style.display = 'flex';
            _playerToggle.innerHTML = '<i class="fas fa-music"></i>';
            _playerToggle.title = 'Mostrar player';
    ...
    if (capoDisplay && capoGrid) {
        ...
        capoGrid.querySelectorAll('.capo-btn-item').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                const formaKey     = document.getElementById('perfKeyDisplay').dataset.formaKey || null;

                // Cifra Club: ao mudar capo, as NOTAS DA CIFRA não mudam.
                // Só o tom real (indicador) muda: tom real = formaKey + novaCasa semitones
                capoDisplay.dataset.capo = novaCasa;
                const capoText = document.getElementById('perfCapoText');
                if (capoText) capoText.textContent = novaCasa === 0 ? 'Sem capo' : `${novaCasa}ª casa`;
                capoGrid.querySelectorAll('.capo-btn-item').forEach(b => b.classList.remove('active'));
                btn.classList.add('active');
                capoGrid.style.display = 'none';

                // Atualiza só o indicador de tom (sem transpor notas)
                if (formaKey) {
                    const idx = SCALE.indexOf(normalizeKey(formaKey));
                    if (idx !== -1) {
                        const realKey = SCALE[(idx + novaCasa) % 12];
                        document.getElementById('perfKeyDisplay').textContent = realKey;
                    }
                }
                // NÃO chama renderPerformanceContent() — notas ficam iguais
            });
        });
    }
